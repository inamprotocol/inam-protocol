...
- **2026-08-25/26: an external audit found 15 issues; #1-#3 are fixed, 12 remain.**
  ...
  - **12 remaining** (not started): reputation date/future-dating safety, provider/requester score conflation, rejected-verification handling, currency conflation in `volumeUsd`, replay/idempotency hardening, external-identity verification depth, key management, job/dispute state-machine completeness, no agent runtime, privacy/access control, search/reputation scaling, doc/version drift. Being worked through in the user's own priority order (P0 first).
- **Test coverage**: 59 Node vitest + 40 Worker vitest + 36 Python pytest = 135 automated tests, all green 2026-08-26 after the audit fixes above.
