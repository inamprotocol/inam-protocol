...
- **`sdk-python/`** — parity Python SDK. **Published and live on PyPI as `inamprotocol@0.2.0`** (confirmed via the public registry). Both language SDKs are now actually installable: `npm install inamprotocol` / `pip install inamprotocol`.
...
Both SDKs are published (npm + PyPI) and Phase 2/3 are complete. Whichever the user picks next: Phase 4 (verified external identity), the root-domain design redo, or writing the OpenWork positioning into README's "relationship to other protocols" section. No default assumed — ask or take the most recently requested thread.
