...
- **2026-08-25/26: an external audit found 15 issues; #1-#6 are fixed, 9 remain.**
  ...
  - **9 remaining** (not started): currency conflation in `volumeUsd`, replay/idempotency hardening, external-identity verification depth, key management, job/dispute state-machine completeness, no agent runtime, privacy/access control, search/reputation scaling, doc/version drift. Being worked through in the user's own priority order.
- **Test coverage**: 64 Node vitest + 46 Worker vitest + 36 Python pytest = 146 automated tests, all green 2026-08-26 after the audit fixes above.
