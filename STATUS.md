...
- **2026-08-25/26: an external audit found 15 issues; #1-#5 are fixed, 10 remain.**
  ...
  - **10 remaining** (not started): rejected-verification handling, currency conflation in `volumeUsd`, replay/idempotency hardening, external-identity verification depth, key management, job/dispute state-machine completeness, no agent runtime, privacy/access control, search/reputation scaling, doc/version drift. Being worked through in the user's own priority order (P0 first, now into P1).
- **Test coverage**: 62 Node vitest + 44 Worker vitest + 36 Python pytest = 142 automated tests, all green 2026-08-26 after the audit fixes above.
