...
- **2026-08-25: an external audit found 15 issues; #1 and #2 are fixed, 13 remain.**
  ...
  - **13 remaining** (not started): verifier independence / registered-verifier enforcement, reputation date/future-dating safety, provider/requester score conflation, rejected-verification handling, currency conflation in `volumeUsd`, replay/idempotency hardening, external-identity verification depth, key management, job/dispute state-machine completeness, no agent runtime, privacy/access control, search/reputation scaling, doc/version drift. Being worked through in the user's own priority order (P0 first).
- **Test coverage**: 56 Node vitest + 37 Worker vitest + 36 Python pytest = 129 automated tests, all green 2026-08-25 after the audit fixes above.
