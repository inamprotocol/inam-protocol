# INAM Protocol — Specification v0.17 (Draft)
...
**Identity layer — complementary.** ERC-8004's Identity Registry is a public, on-chain, chain-native directory. INAM has no such primitive and does not want one (§0: identity issuance and discovery are out of scope). The natural composition is an ERC-8004 identity that *also* carries an INAM ID, with INAM execution receipts referencing the on-chain identity. INAM's `linked` map (§2) is the mechanism, with one gap: an ERC-8004 identity is an EVM address (secp256k1), and §2.1's link-challenge currently supports only `ed25519` and `p256`. A `secp256k1` `keyType` plus an `erc8004_id` link protocol would close it — a scoped increment (`@noble/curves` already ships secp256k1; the work is a keccak/secp256k1 verify path and one enum value), deliberately **not** in v0.17.
